...
def _build_analysis_prompt(
    ...
    for i, p in enumerate(proposals):
        ...
        # Risk flags from exclusions detector (PythonProject5)
        risk_flags   = p.get("_risk_flags", [])
        safety_rating = p.get("_safety_rating", 9.0)
        multidim     = p.get("_multidim", {})

        risk_block = ""
        if risk_flags:
            risk_block = f"  ⚠️ RISK FLAGS: {'; '.join(risk_flags)}\n"
        safety_block = f"  Safety Rating: {safety_rating}/10\n" if safety_rating < 9.0 else ""

        dim_block = ""
        if multidim:
            dim_block = (
                f"  Βαθμολογία: Κάλυψη={multidim.get('coverage_breadth','?')} | "
                f"Οικον.Προστασία={multidim.get('financial_protection','?')} | "
                f"Πρόληψη={multidim.get('preventive_care','?')} | "
                f"Προσβασιμότητα={multidim.get('accessibility','?')}\n"
            )
        tag = " ← ΠΡΟΤΕΙΝΟΜΕΝΟ" if i == recommended_idx else ""
        summary = (
        ...
        ) + dim_block + risk_block + safety_block
        if p.get("key_notes"):
    ...
    L = get_profile(lang)
    lang_instr = L.analysis_instruction

    # Terms/wording findings block
    terms_block = ""
    if terms_results:
        lines = ["\nΕΥΡΗΜΑΤΑ ΑΠΟ POLICY WORDING (Γενικοί Όροι — Πραγματικές Εξαιρέσεις):"]
        for ins, tdata in terms_results.items():
            excl = tdata.get("exclusions", [])
            if excl:
                lines.append(f"\n  {ins}:")
                for e in excl[:6]:  # max 6 per insurer in prompt
                    sev = e.get("severity","MEDIUM")
                    desc = e.get("description","")
                    wording = e.get("exact_wording","")
                    lim = e.get("limit_value","")
                    lines.append(
                        f"    [{sev}] {desc}"
                        + (f" | Wording: \"{wording}\"" if wording else "")
...
Αν υπάρχουν ευρήματα από το policy wording, ενσωμάτωσέ τα στην ανάλυση (ειδικά στα key_concerns).
...
def generate_recommendation_analysis(
    ...
    prompt = _build_analysis_prompt(
        proposals, recommended_idx, client_name, client_members,
        lang=lang, terms_results=terms_results or {}
    )
